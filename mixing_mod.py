...
class LinearCombinationCalculator(Calculator):
    ...
    def __init__(self, calcs, weights, atoms=None, ignore_not_implemented=False):
        ...
        ## added in modification
        if you want to derive properties which are only available in some calculator, use ignore_not_implemented=True
        """
        ...
        if(ignore_not_implemented):
            common_properties = set.union(*(set(calc.implemented_properties) for calc in calcs))
            #common_properties = [ set(calc.implemented_properties) for calc in calcs ]
        else:
            common_properties = set.intersection(*(set(calc.implemented_properties) for calc in calcs))
        
        #print("all implemented properties: ", common_properties)
        ...
        self.ignore_not_implemented = ignore_not_implemented
        ...
        for w, calc in zip(self.weights, self.calcs):
            ...
            if calc.calculation_required(atoms, properties):
                ...
                if(self.ignore_not_implemented):
                    props = set.intersection( set(calc.implemented_properties), self.implemented_properties)
                    #print("all properties: ", properties)
                    #print(calc.name)
                    #print("properties for this calculator: ", props)
                    
                    #print(calc.name)
                    if(calc.name=="lammpslib"): # CAUTION: not normal lammpslib but lammpslib3.py
                        
                        charges = self.results["charges"] # charges from PFP
                        
                        calc.reset_additional_command()
                        for iatom, qi in enumerate(charges):
                            cmd = "set atom " + str(iatom+1) + " charge " + str(qi)
                            calc.set_additional_command(cmd)

                        # for debug
                        calc.set_additional_command("thermo_style custom pe pxx emol ecoul epair evdwl elong")
                    calc.calculate(atoms, props, system_changes)

                    for k in props:
                        #print("calc, property: ", calc.name, k)
...
class SumCalculator(LinearCombinationCalculator):
    ...
    def __init__(self, calcs, atoms=None, ignore_not_implemented=False):
        """Implementation of sum of calculators.
        ...
        weights = [1.] * len(calcs)
        super().__init__(calcs, weights, atoms, ignore_not_implemented)
